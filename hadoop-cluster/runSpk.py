...
def runSpark(filename, inputMap):  
  ...
  if len(filename) > 0:
    ...
    if s == "py" or s == "spark":
      ...
      uq = "-" + str(uuid.uuid1()) 
      #print("uuid", uq)      
      cmd7 = pfx + " -- spark-submit --executor-memory " + executor_memory + " --executor-cores " + executor_cores + " --master " + spkmaster + " " + inbox + "/" + f 
      #print(cmd7)
      ...
      #md8 = "kubectl cp " + f + uq + ".result " + nmspace + "/" + hmaster + ":" + outbox
      #rint(cmd8)      
      #s.system(cmd8)      
      #etLog(f +  uq + ".log")
      #s.system("kubectl cp " + f +  uq + ".log " + nmspace + "/" + hmaster + ":" + outbox)        
    else:
      pass
