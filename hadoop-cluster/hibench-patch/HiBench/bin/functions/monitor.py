...
logging.basicConfig(filename='/root/debug.log', level=logging.DEBUG, format=LOG_FORMAT, datefmt=DATE_FORMAT)
...
def log(*x, **kw):
 with open("/root/probe.log", kw.get("mode","a")) as f:
...
')"""
    ...
    def run(self):
        ...
        script = self.template_debug.format(func_template=func_template,
        ...
        with self.ssh_client(self.host, "python -u -c \"{script}\"".format(script=s)) as f:
            ...
            try:
                ...
                #if port_line.startswith("Warning"):
                #    port_line = f.readline()
    ...
    def ssh_client(self, host, shell):
        with open(os.devnull, 'rb', 0) as DEVNULL:
            with BashSSHClientMixin.ssh_lock:
                self.proc = subprocess.Popen(["ssh", "-q", host, shell], bufsize=1, 
