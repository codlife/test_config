...
if len(sys.argv) >= 2:
  nmspace = sys.argv[1]
print(nmspace)
'''os.system("mkdir " + nmspace)
os.system("cp " + deployFile + " " + undeployFile + " " + master_cfg + " " + worker_cfg + " " + master_svc + " " + nmspace_hp + " " + nmspace)

def addPfx(nm, pfx):
  print("./" + pfx + "/" + nm)
  return "./" + pfx + "/" + nm

deployFile = addPfx(deployFile, nmspace)
undeployFile = addPfx(undeployFile, nmspace)
master_cfg = addPfx(master_cfg, nmspace)
worker_cfg = addPfx(worker_cfg, nmspace)
old_master_svc = master_svc
master_svc = addPfx(master_svc, nmspace)
nmspace_hp = addPfx(nmspace_hp, nmspace)'''
...
def chmst_svc(mst_svc, offset):
  ...
  for i in range(len(cnt)):
    if cnt[i].find("ports") != -1:
      ...
      ac = cnt[i].split(":")
      cnt[i] = ac[0] + ": " + str(int(ac[1].strip()) + offset) + "\n"    
    else:
      pass
...
nm = os.popen("kubectl get namespace")
nm = nm.read()
nm = [l.split()[0] for l in nm.split("\n")[1:] if len(l) > 0]
print("nm", nm)
if nmspace == "hadoop" and nmspace not in nm:
  pass
else:
  chmst_svc(master_svc, int(sys.argv[2]))
  #chmst_svc(old_master_svc)
